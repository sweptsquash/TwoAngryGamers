...
if [ $(docker ps | grep -c "${DOCKER_CONTAINER}") -lt 1 ]; then
    ...
    docker run -t -d --restart=always --name "${DOCKER_CONTAINER}" -w /var/www/twoangrygamers.test -v $(pwd):/var/www/twoangrygamers.test:delegated alpine sh
fi
...
echo "Waiting for POD & Container to start..."
sleep 30
